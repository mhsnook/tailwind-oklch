...
export default function HueControls() {
	...
	return (
		<div className="flex gap-6 flex-wrap p-6 bg-composed bg-l-20 bg-c-lo bg-h-primary rounded-xl border border-lo-lo-primary">
			{HUES.map((hue) => (
				<div key={hue.name} className="flex flex-col gap-1.5 min-w-[180px]">
					...
					<SliderControl
						...
						label={`${hue.name} hue`}
					/>
					<div className="font-mono text-[0.8rem] text-mhi-mid-primary">{values[hue.name]}°</div>
				</div>
			))}
		</div>
	)
